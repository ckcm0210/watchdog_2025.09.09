...
def generate_html(events=None):
    ...
    try:
        ...
        html.append('  }')
        html.append('  ')
        html.append('  // 全局函數：比較兩個公式並生成帶有顏色標記的 diff HTML')
        ...
        html.append('    }')
        html.append('    ')
        html.append('    // 簡單的字符級別 diff 算法')
        html.append('    function getCharDiff(str1, str2) {')
        html.append('      const result = [];')
        html.append('      const maxLen = Math.max(str1.length, str2.length);')
        html.append('      ')
        html.append('      let i = 0;')
        html.append('      while (i < maxLen) {')
        html.append('        const char1 = str1[i] || "";')
        html.append('        const char2 = str2[i] || "";')
        html.append('        ')
        html.append('        if (char1 === char2) {')
        html.append('          // 相同字符')
        html.append('          result.push({type: "same", char: char1});')
        html.append('        } else if (char1 && !char2) {')
        html.append('          // 舊公式有，新公式沒有 - 刪除')
        html.append('          result.push({type: "deleted", char: char1});')
        html.append('        } else if (!char1 && char2) {')
        html.append('          // 新公式有，舊公式沒有 - 新增')
        html.append('          result.push({type: "added", char: char2});')
        html.append('        } else {')
        html.append('          // 字符不同 - 標記為刪除舊的，新增新的')
        html.append('          result.push({type: "deleted", char: char1});')
        html.append('          result.push({type: "added", char: char2});')
        html.append('        }')
        html.append('        i++;')
        ...
        html.append('    }')
        html.append('    ')
        html.append('    const diff = getCharDiff(oldStr, newStr);')
        html.append('    let html = "";')
        html.append('    ')
        html.append('    // 將連續的相同類型字符組合在一起')
        html.append('    let currentType = null;')
        html.append('    let currentChars = "";')
        html.append('    ')
        html.append('    function flushCurrent() {')
        html.append('      if (currentChars) {')
        html.append('        const escapedChars = currentChars.replace(/</g, "&lt;").replace(/>/g, "&gt;");')
        html.append('        switch (currentType) {')
        html.append('          case "same":')
        html.append('            html += "<span class=\\"formula-diff-unchanged\\">" + escapedChars + "</span>";')
        html.append('            break;')
        html.append('          case "deleted":')
        html.append('            html += "<span class=\\"formula-diff-deleted\\">" + escapedChars + "</span>";')
        html.append('            break;')
        html.append('          case "added":')
        html.append('            html += "<span class=\\"formula-diff-added\\">" + escapedChars + "</span>";')
        html.append('            break;')
        html.append('        }')
        html.append('        currentChars = "";')
        html.append('      }')
        html.append('    }')
        html.append('    ')
        html.append('    diff.forEach(item => {')
        html.append('      if (item.type !== currentType) {')
        html.append('        flushCurrent();')
        html.append('        currentType = item.type;')
        html.append('      }')
        html.append('      currentChars += item.char;')
        html.append('    });')
        html.append('    ')
        html.append('    flushCurrent();')
